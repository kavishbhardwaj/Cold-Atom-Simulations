...
not from the vapour containing only one isotope. the loading/loss model loading must therefore
...
Those fractions are provenance data only in the OBE benchmark; they are not yet used to
predict loading.
...
its own hyperfine data plus the coherent multilevel Hamiltonian and decay graph.
...
| Isotope/line | Generated basis | Effective MOT | Rate-equation MOT | Sparse OBE operators | Polarization-gradient / gray molasses |
|---|---:|---:|---:|---:|---:|
| 87Rb D2 | 24 states | yes | F=2→F′=3; repump F=1→F′=2 | yes; research validation pending | adiabatic F=2→F′=3 populations |
| 87Rb D1 | 16 states | line benchmark only | no | yes; no gray-molasses validation | not implemented |
| 85Rb D2 | 36 states | yes | F=3→F′=4; repump F=2→F′=3 | yes; research validation pending | not validated |
| 85Rb D1 | 24 states | line benchmark only | no | yes; no gray-molasses validation | not implemented |
...
`FineStructureLine.model_support` exposes that distinction programmatically.
...
|---|---|---|
| 87Rb D2 | Standard F=2→F'=3 MOT cooling; F=1→F'=2 repump | Default effective/rate-equation reference |
...
| 85Rb D2 | Standard F=3→F'=4 MOT cooling; F=2→F'=3 repump | Genuine generated I=5/2 rate-equation basis |
