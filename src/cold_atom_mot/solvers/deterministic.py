...
def integrate_trajectory(force_model, position, velocity, duration, *, max_step=1e-5, rtol=1e-7, atol=1e-10) -> Trajectory:
    ...
    def derivative(time, state):
        return np.concatenate([state[3:], force_model.force(state[:3], state[3:], time) / force_model.atom.mass])

    solution = solve_ivp(derivative, (0.0, duration), initial, max_step=max_step, rtol=rtol, atol=atol)
