"""Phase-resolved Level-D polarization-gradient cooling.
...
def coherent_six_beam_field(wave_number: float, saturation_per_beam: float, phases=None):
    ...
    for axis_index, (axis, helicity) in enumerate(zip(np.eye(3), (1, 1, -1))):
        for direction_sign in (-1, 1):
            ...
            beams.append(CoherentBeam(direction, circular_polarization(direction, helicity),
                                      saturation_per_beam, wave_number, phases[index]))
    return tuple(beams)
...
class PolarizationGradientModel:
    ...
    def __init__(self, gamma, detuning, wave_number, beams, *, ground_g_factor=0.499836,
                 excited_g_factor=0.667,
                 magnetic_field_t=(0.0, 0.0, 0.0), quantization_axis=(0.0, 0.0, 1.0)):
        if gamma <= 0 or detuning == 0:
            raise ValueError("gamma must be positive and detuning non-zero")
        self.gamma, self.detuning, self.wave_number = gamma, detuning, wave_number
        self.beams = tuple(beams)
        self.ground_g_factor = ground_g_factor
        self.excited_g_factor = excited_g_factor
        self.magnetic_field_t = np.asarray(magnetic_field_t, float)
        self.quantization_axis = unit(quantization_axis)
        self.strength = np.zeros((5, 7, 3))
        for gi, m in enumerate(self.ground_m):
            for qi, q in enumerate((-1, 0, 1)):
                ep = m + q
                if abs(ep) <= 3:
                    self.strength[gi, ep + 3, qi] = clebsch_gordan(2, int(m), 1, q, 3, int(ep)) ** 2
    ...
    def polarization_components(self, position):
        field = self.electric_field(position)
        intensity = float(np.vdot(field, field).real)
        if intensity < 1e-30:
            return intensity, {-1: 1 / 3, 0: 1 / 3, 1: 1 / 3}
        return intensity, spherical_fractions(field, self.quantization_axis)
