"""Generate reproducible Level-D polarization-gradient diagnostics."""
import json
...
def build(saturation=0.08, detuning_gamma=-3, bias_t=0):
    ...
    return PolarizationGradientModel(ATOM.gamma, detuning_gamma * ATOM.gamma,
                                     ATOM.wave_number, beams, magnetic_field_t=[0, 0, bias_t])
...
def main():
    ...
    plt.rcParams["svg.hashsalt"] = "cold-atom-mot-phase4"
    ...
    plt.figure(figsize=(7.5, 4.8)); plt.plot(velocities, pg_force * 1e22, "o-", label="Level D dipole/pumping")
    ...
    plt.xlabel("Integration samples per λ period"); plt.ylabel("Mean Fx ($10^{-22}$ N)"); plt.title("Phase-4 force-grid convergence")
    ...
    metadata = {"simulation_version": __version__, "model_fidelity": "Level D adiabatic F=2 to F'=3 population Sisyphus model",
    ...
    np.savez_compressed(OUTPUT / "phase4_reference.npz", x_m=x, polarization_minus=fractions[-1], polarization_pi=fractions[0],
