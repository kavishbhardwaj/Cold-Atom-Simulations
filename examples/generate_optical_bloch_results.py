"""Generate Two-level OBE reduced-OBE validation and conditioned trend figures."""
...
OUTPUT = ROOT / "results" / "optical_bloch"
EFFECTIVE_CONFIG = ROOT / "configs" / "rb87_d2_mot.yaml"
PHASE3 = ROOT / "configs" / "rb87_d2_two_level_obe.yaml"
...
def main() -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "cold-atom-mot-obe"
    ...
    effective_config = load_config(EFFECTIVE_CONFIG)
    ...
    for i, power in enumerate(powers):
        for j, delta in enumerate(detuning_scan):
            varied = copy.deepcopy(effective_config)
    ...
    reference_waist = effective_config["laser"]["waist_m"]
    reference_power = effective_config["laser"]["power_per_beam_w"]
    for waist in waists:
        fixed_power = copy.deepcopy(effective_config)
    ...
    metadata = {
        ...
        "model_fidelity": "Coherent model reduced single-transition OBE plus conditioned effective-model scans",
        ...
        "scope_warning": "Only 87Rb D2 has effective-model/B MOT support; all entries support constant comparison only.",
    }
    np.savez_compressed(
        OUTPUT / "optical_bloch_reference.npz",
