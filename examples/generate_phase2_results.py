"""Generate reproducible Level-B multilevel rate-equation diagnostics."""
...
CONFIG_PATH = ROOT / "configs" / "rb87_phase2_rate_equation.yaml"
...
def main() -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "cold-atom-mot-phase2"
    ...
    level_a_config["model"] = "level_a_effective_two_level"
    level_a = build_effective_model(level_a_config)
    force_effective = np.array([level_a.force([x, 0, 0], [0, 0, 0])[0] for x in position])
    plt.figure(figsize=(7.2, 4.8))
    plt.plot(position * 1e3, force_effective * 1e21, label="Level A: effective two-level", linewidth=2)
    plt.plot(position * 1e3, force_multilevel * 1e21, label="Level B: 24-state rate equation", linewidth=2)
    plt.axhline(0, color="black", linewidth=0.8)
    plt.xlabel("x (mm)"); plt.ylabel("$F_x$ ($10^{-21}$ N)")
    plt.title("Restoring-force fidelity comparison")
    plt.legend()
    save("force_level_a_vs_b")
    ...
    plt.title("Level-B damping force near zero velocity")
    ...
    np.savez_compressed(
        OUTPUT / "phase2_reference.npz",
        position_m=position,
        force_level_a_n=force_effective,
        force_level_b_n=force_multilevel,
